...
if 0 < 2
{
	MsgBox, 16, SciTE4AutoHotkey Toolbar, This script should not be ran independently.
	ExitApp
}
...
FileRead, SciTEVLocal, %LocalSciTEPath%\$VER

; If SciTEVLocal is set and SciTEVLocal isn't the same as SciTEVRunning
if SciTEVLocal && (SciTEVLocal != SciTEVRunning)
	; Run UpdateProfile
	gosub UpdateProfile

; If not portable and LocalPropsPath doesn't exist or SciTEVLocal is empty
if !IsPortable && (!FileExist(LocalPropsPath) || !SciTEVLocal)
{
...
SciTEVLocalInt := Util_VersionTextToNumber(SciTEVLocal)
...
; Create tool image list
_ToolIL := IL_Create()
; Set iconlibrary name
...
; Loop through toolbar properties
Loop, Parse, ToolbarProps, `n, `r
{
	; Trim current line
	curline := Trim(A_LoopField)
	; If current line is blank or if it starts with comment, next iteration
	if (curline = "") || SubStr(curline, 1, 1) = ";"
		continue
	
	; If current line starts with double dash
	if SubStr(curline, 1, 2) = "--"
	{
		; Add a tripe dash to tool buttons
		_ToolButs .= "---`n"
		; increment ntools and start next iteration
		ntools++
		continue
	; Else if current line starts with a single dash
	}else if (SubStr(curline, 1, 1) = "-"){
		; Add a single dash to tool buttons
		_ToolButs .= "-`n"
		; increment ntools and start next iteration
		ntools++
		continue
	; Else if current line does not match regex or if varz1 is blank, next iteration
	}else if !RegExMatch(curline, "^=(.*?)\x7C(.*?)(?:\x7C(.*?)(?:\x7C(.*?))?)?$", varz) || varz1 = ""
		continue

	; Increment tools
	ntools ++
	; Search for grave accent and warn if found in varz1
	IfInString, varz1, `,
	{
		MsgBox, 16, SciTE4AutoHotkey Toolbar, A tool name cannot contain a grave accent! Specified:`n%varz1%
		ExitApp
	}

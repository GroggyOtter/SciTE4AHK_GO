/*! 
TillaGoTo for SciTe4AHK_GO
Function, Hotkey, Label, and Class detector
Updated by the GroggyOtter
Last updated: 17Oct2018
Updates (if any) can be found at:
https://github.com/GroggyOtter/SciTE4AHK_GO

Originally created by TheGood
Last updated: 30Dec2010
Original changelog and help can be found in the thread:
http://www.autohotkey.com/forum/viewtopic.php?t=41575
*/

; Get SciTE object
sciteObj := GetSciTEInstance()
if !sciteObj
{
    MsgBox, 16, TillaGoto, Cannot find SciTE4AHK_GO!
    ExitApp
}

; Get SciTE window handle
hwndSciTE := sciteObj.SciTEHandle

; Read global settings from tillagoto.properties file using SciTE's property system
	; Show/Hide systray icon
	bTrayIcon           := sciteObj.ResolveProp("tillagoto.show.tray.icon") + 0
	; Width of GUI
	iGUIWidth           := sciteObj.ResolveProp("tillagoto.gui.width") + 0
	; Number of listbox rows
	iGUIHeight           := sciteObj.ResolveProp("tillagoto.gui.height") + 0
	; Number of listbox rows
	iGUIRows          	:= sciteObj.ResolveProp("tillagoto.gui.row") + 0
	; GUI Margin
	iMargin             := sciteObj.ResolveProp("tillagoto.gui.margin") + 0
	; GUI transparency
	iTransparency       := sciteObj.ResolveProp("tillagoto.gui.transparency") + 0
	; GUI control padding
	iPad 				:= sciteObj.ResolveProp("tillagoto.gui.pad") + 0
	; Dock GUI on left|right side
	bPosLeft            := sciteObj.ResolveProp("tillagoto.gui.posleft") + 0
	; Resize GUI width to fit all text
	bWideView           := sciteObj.ResolveProp("tillagoto.gui.wide.view") + 0
	; #Include filename alignment
	iAlignFilenames     := sciteObj.ResolveProp("tillagoto.gui.align.filenames") + 0
	; GUI background color
	cGUIBG := cGUIBGOrig := sciteObj.ResolveProp("tillagoto.gui.bgcolor")
	; Control background color
	cControlBG := cControlBGOrig := sciteObj.ResolveProp("tillagoto.gui.controlbgcolor")
	; Control foreground (text) color
	cControlFG := cControlFGOrig := sciteObj.ResolveProp("tillagoto.gui.controlfgcolor")
	; Font size
	iControlFontSize    := sciteObj.ResolveProp("tillagoto.gui.font.size") + 0
	; Font typeface
	fControlFont        := sciteObj.ResolveProp("tillagoto.gui.font")
	; Default sort method order|asc|desc
	iSortEntries        := sciteObj.ResolveProp("tillagoto.gui.sort.entries") + 0
	; Hotkey: Show GUI
	uSummonGUI          := sciteObj.ResolveProp("tillagoto.hk.summon.gui")
	; Hotkey: Previous view 
	uGoBack             := sciteObj.ResolveProp("tillagoto.hk.go.back")
	; Hotkey: Next view
	uGoForward          := sciteObj.ResolveProp("tillagoto.hk.go.forward")
	; Hotkey: Quick goto
	uGotoDef            := sciteObj.ResolveProp("tillagoto.hk.goto.def")
	; Filter out comments
	bFilterComments     := sciteObj.ResolveProp("tillagoto.filter.comments") + 0
	; Quick mode (advise not to use)
	bQuickMode          := sciteObj.ResolveProp("tillagoto.quick.mode") + 0
	; Exit when editor exits
	bQuitWithEditor     := sciteObj.ResolveProp("tillagoto.quit.with.editor") + 0
	; Match search anywhere in GUI list
	; Multiple words will be matched in any order
	; Ex: () cat dog would match the function DogCat()
	bMatchEverywhere    := sciteObj.ResolveProp("tillagoto.match.everywhere") + 0
	; Use middle mouse to call GUI
	bUseMButton         := sciteObj.ResolveProp("tillagoto.use.mbutton") + 0
	; Middle mouse hold time to close GUI
	iCancelWait         := sciteObj.ResolveProp("tillagoto.cancel.timeout") + 0
	; Setting for searching through #Include files
	iIncludeMode        := sciteObj.ResolveProp("tillagoto.include.mode") + 0
	; File caching setting
	bCacheFiles         := sciteObj.ResolveProp("tillagoto.cache.files") + 0
	; Allo TillaGoto specific directives
	bDirectives         := sciteObj.ResolveProp("tillagoto.directives") + 0

_AhkScriptIsActive()
{
    global sciteObj
    return _SciTEIsActive() ;&& sciteObj.ResolveProp("Language") = "ahk1" ; Alternate test: FileExt = "ahk"
}
...
; Necessary for the conditional hotkey/hotstring expression to be registered
    ...
    Menu, Tray, Icon, %A_ScriptDir%\..\TillaGoto.ico
    Menu, Tray, Tip, TillaGoto for SciTE4AHK_GO
    Menu, Tray, Add, Dark Mode, DarkMode
    Menu, Tray, Add, Open GitHub, GitHub
    ...
    Gui, Add, Edit, xm ym h%iGUIRows% w%iGUIWidth% vtxtSearch gtxtSearch_Event hwndhtxtsearch
    Gui, Add, Text, % "xm+" iPad " y+" iMargin, Sort:
    Gui, Add, Radio, % "x+" iPad*2 " vGuiRadio1 gGuiSortList Checked", In Order
    Gui, Add, Radio, % "x+" iPad " vGuiRadio2 gGuiSortList", % "Asc" Chr(0x2191)		; 0x2191 = Up arrow
    Gui, Add, Radio, % "x+" iPad " vGuiRadio3 gGuiSortList", % "Desc" Chr(0x2193)	; 0x2193 = Down arrow
    ;sortOpt := iSortEntries ? "Sort" : ""
    ; Add tabs here for sorting by type
    ;Gui, Add, Tab, whxy
    Gui, Add, ListBox, xm y+0 wp r%iGUIRows% vlblList glblList_Event hwndhlblList +HScroll +256 ;LBS_NOINTEGRALHEIGHT
...
DarkMode:
	if darkModeToggle is not Number
		darkModeToggle := 0
	...
	if (darkModeToggle > 0){
		; Check tray menu
	    Menu, Tray, Check, Dark Mode
		; Set dark colors and update everything
		Gui, Color, 0x333333, 0x111111
		Gui, Font, c0x00FF00
		GuiControl, Font, lblList
		Gui, Font, c0xFFFF00
		GuiControl, Font, txtSearch
	}Else{
		; Uncheck tray menu
	    Menu, Tray, Uncheck, Dark Mode
		; Set original colors and update everything
		Gui, Color, % cGUIBG, % cControlBG
		Gui, Font, c%cControlFG%
		GuiControl, Font, lblList
		Gui, Font, c%cControlFG%
		GuiControl, Font, txtSearch
	}
...
GitHub:
    Run, % "https://github.com/GroggyOtter/SciTE4AHK_GO"
Return
...
SummonGUI:
    ...
    If bWideView {
        ...
        bVScroll := (sLabels0 + sFuncs0 > iGUIRows)
    ...
    ;Check if hscroll will appear and adjust height to make sure %iGUIRows% items are visible
    SendMessage, 403, 0, 0,, ahk_id %hlblList% ;LB_GETHORIZONTALEXTENT
    ControlMove,,,,, iGUIRows * iGUIItemHeight + (ErrorLevel > iW ? SM_CYHSCROLL : 0) + 4, ahk_id %hlblList%
    ...
    Gui, Show, AutoSize x%iX% y%iY%, TillaGoto GUI
...
GuiSortList:
;vlblList glblList_Event hwndhlblList
	Gui, Submit, NoHide
	ControlGet, testVar, List,, ahk_id %hlblList%
	MsgBox, % testVar
Return
...
LaunchFile(sFilePath, iLine) {
    Global cSci, sciteObj
    
    ;Open the file in SciTE
    sciteObj.OpenFile(sFilePath)
...
AnalyzeScript:
    ...
    If (iIncludeMode & 0x00000010) {
        ...
        UsedAhkPath := sciteObj.ResolveProp("AutoHotkey")
...
;                         . "(?=([[:blank:]]*[\r\n]|[[:blank:]]+\Q" v "\E))", t, i)
        RegExMatch(s, "O)(^|\n|\r\n)\s*([\w\Q!@#$^&*()-=_+{}[]\E]+:{1})(?=(\s+|\n|\r\n))", match, i)
		t := Trim(match.Value(), "`n`r`t ")
        i := Match.Pos() + Match.Len()
...
;                         . "(?=\(.*?\)(\s+\Q" u "\E.*?[\r\n]+)*?\s+\{)", t, i)
        ...
        ;{   ;Increment counter
            ...
            sFuncs%sFuncs0% := t
            ;MsgBox, % "sFuncs%sFuncs0%: " sFuncs%sFuncs0% "`nsFuncs0: " sFuncs0
        ;}
...
CreateList(filter = "") { 
    ...
    GuiControlGet, testVar,, ahk_id %hlblList%
